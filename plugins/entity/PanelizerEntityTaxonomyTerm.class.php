  ...
  public function settings_form(&$form, &$form_state) {
    ...
    if ($warn) {
      ...
      if (!empty($task['disabled'])) {
        drupal_set_message(t('The taxonomy term template page is currently not enabled in page manager. You must enable this for Panelizer to be able to panelize taxonomy terms using the "Full page override" view mode.'), 'warning', FALSE);
      }

      $handler = page_manager_load_task_handler($task, '', 'term_view_panelizer');
      if (!empty($handler->disabled)) {
        drupal_set_message(t('The panelizer variant on the taxonomy term template page is currently not enabled in page manager. You must enable this for Panelizer to be able to panelize taxonomy terms using the "Full page override" view mode.'), 'warning', FALSE);
      }
    }
  }
